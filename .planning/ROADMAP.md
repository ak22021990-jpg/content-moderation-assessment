...
- [x] 05-01-PLAN.md — Client Submission Pipeline + Happy Path UI: submission.js payload assembly + Web Crypto HMAC, dedup.js SHA-256 rewrite, ScoreboardScreen overlay (idle→submitting→success), SubmitDoneScreen, screen enum, localStorage flag, sessionStorage clear
- [x] 05-02-PLAN.md — Apps Script Backend + Retry Logic + Error UX: doPost webhook (HMAC/origin/rate-limit/dedup/row write), exponential backoff retry, error overlay with manual retry button
- [x] 05-03-PLAN.md — Formspree Fallback + Final Coverage: @formspree/react env-toggle, integration tests, full suite green
